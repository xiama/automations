...
def application(environ, start_response):
	...
	if environ['PATH_INFO'] == '/health':
		...
		response_body = '\n'.join(response_body)
                ...
                command1 = "echo 'testing' >/tmp/wsgi_tmp_test"
                response_body = response_body + "Command 1: %s" %(command1) + "\n"
                (ret1, output) = commands.getstatusoutput(command1)
                response_body = response_body + output + "\n"

                command2 = "echo 'testing' >/var/tmp/wsgi_var_tmp_test"
                response_body = response_body + "Command 2: %s" %(command2) + "\n"
                (ret2, output) = commands.getstatusoutput(command2)
                response_body = response_body + output + "\n"

                command = "ls -l /tmp"
                response_body = response_body + "Command: %s" %(command) + "\n"
                (ret_tmp, output) = commands.getstatusoutput(command)
                response_body = response_body + output + "\n"
                ...
                command1 = "ls -l /tmp/wsgi_tmp_test && ls -l /var/tmp/wsgi_var_tmp_test"
                response_body = response_body + "Command 1: %s" %(command1) + "\n"
                (ret1, output) = commands.getstatusoutput(command1)
                response_body = response_body + output + "\n"
