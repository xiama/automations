...
class OpenShiftTest(rhtest.Test):
    ...
    def initialize(self):
        ...
        self.databases = {
            ...
            'postgresql-9.2': {'check':'psql --version',
                           'cmd': 'echo "*:PORT:DATABASE:USERNAME:PASSWORD">$HOME/.pgpass;chmod 0600 $HOME/.pgpass;echo "\d" | psql -w -h HOSTNAME --port PORT -U USERNAME DATABASE'},
...
class RhcPortForward(OpenShiftTest):
    ...
    def check_port(self, serv, host, port):
        ...
        if serv in ('haproxy'):
            ...
            db_info = OSConf.get_embed_info(self.app_name, common.cartridge_types['postgresql-9.2'])
            ...
            return common.grep_web_page("%s:%s" % (host, port), "([Oo]pen[Ss]hift)|(Statistics Report)")
        else:
    ...
    def test_method(self):
        ...
        if self.scalable:
            self.info("Embed Postgresql-9.2 the database cartridges to the scalable app")

            ret = common.embed(self.app_name,
                           'add-%s'%common.cartridge_types['postgresql-9.2'],
                           self.config.OPENSHIFT_user_email,
                           self.config.OPENSHIFT_user_passwd)
            self.assert_equal(ret, 0, "Embeding of postgresql-9.2 should pass.")
