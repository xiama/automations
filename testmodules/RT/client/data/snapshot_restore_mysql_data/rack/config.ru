...
target_file = ENV['OPENSHIFT_DATA_DIR'] + "rack_data_test"
  ...
  response_body = "Welcome~\n"
    ...
    begin
      ...
      while row = res.fetch_row do
        response_body = response_body + row.join(", ") + "\n"
      end
      res.free
    rescue MysqlError => e
      response_body = response_body + "Error code: #{e.errno}"
      response_body = response_body + "Error message: #{e.error}"
    ensure
      dbh.close if dbh
    end
...
map '/create' do
  create = proc do |env|
    ...
    begin
      ...
      dbh.query("INSERT INTO ucctalk (speaker,title) VALUES ('speaker1', 'title1')")
      response_body = response_body + "#{dbh.affected_rows} rows were inserted:" + "\n"
      res = dbh.query("SELECT * FROM ucctalk")
      while row = res.fetch_row do
        response_body = response_body + row.join(", ") + "\n"
      end
...
map '/modify' do
  modify = proc do |env|
    ...
    begin
      ...
      dbh.query("DROP TABLE IF EXISTS ucctalk")
      dbh.query("CREATE TABLE ucctalk (speaker CHAR(30), title CHAR(60))")
      dbh.query("INSERT INTO ucctalk (speaker,title) VALUES ('speaker2', 'title2')")
      response_body = response_body + "#{dbh.affected_rows} rows were inserted:" + "\n"
      res = dbh.query("SELECT * FROM ucctalk")
      while row = res.fetch_row do
        response_body = response_body + row.join(", ") + "\n"
