...
https://tcms.engineering.redhat.com/case/167904/
"""
import os
import common
import OSConf
...
class OpenShiftTest(rhtest.Test):
    ...
    def initialize(self):
        ...
        self.summary = "[rhc-cartridge]snapshot/restore big data to existing app"
...
class BigDataTest(OpenShiftTest):
    ...
    def test_method(self):
        ...
        elif self.test_variant in ('jbossas', 'jbosseap','jbossews','jbossews-1.0','jbossews-2.0'):
        ...
        url = self.url_dict[self.test_variant]["show"]
        ret = common.grep_web_page(url, "The bigfile exists", "-H 'Pragma: no-cache' -L", 5, 3)
        ...
        self.step("Delete the bigfile")
        url = self.url_dict[self.test_variant]["delete"]
        ret = common.grep_web_page(url, "The bigfile has been deleted", "-H 'Pragma: no-cache' -L", 5, 3)
        self.assert_equal(ret, 0, "Failed to delete the bigfile")

        self.step("Check the bigfile doesn't exist")
        url = self.url_dict[self.test_variant]["show"]
        ret = common.grep_web_page(url, "The bigfile doesnot exist", "-H 'Pragma: no-cache' -L", 5, 3)
        self.assert_equal(ret, 0, "The bigfile still exists")
        ...
        ret = common.command_get_status(cmd, timeout=-1)
        self.assert_equal(ret, 0, "Failed to restore the existing app")
