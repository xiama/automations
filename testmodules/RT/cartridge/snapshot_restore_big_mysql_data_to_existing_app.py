...
class BigMysqlDataTest(OpenShiftTest):
    ...
    def test_method(self):
        ...
        self.step("Delete the data in MySQL")
        url = self.url_dict[self.test_variant]["delete"]
        ret = common.grep_web_page(url, "All the records have been deleted from mysql database",
                                    "-H 'Pragma: no-cache' -L", 5, 6, True)
        self.assert_equal(ret, 0, "Failed to delete the MySQL data")

        self.step("Check MySQL data doesn't exist")
        url = self.url_dict[self.test_variant]["show"]
        ret = common.grep_web_page(url, 'There is no record in database', "-H 'Pragma: no-cache' -L", 5, 6)
        self.assert_equal(ret, 0, "MySQL data still exists")
