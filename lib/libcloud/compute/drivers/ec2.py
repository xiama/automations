...
class EC2NodeDriver(NodeDriver):
    ...
    def _to_image(self, element):
        n = NodeImage(id=findtext(element=element, xpath='imageId',
                      ...
                      driver=self.connection.driver,
                      extra={
                          'name': findattr(element=element,
                                            xpath="name",
                                            namespace=NAMESPACE),
                          'tag': findattr(element=element,
                                            xpath="tagSet/item/value",
                                            namespace=NAMESPACE),
    ...
    def list_images(self, location=None):
        ...
        return images
    """
    def list_images(self, location=None, ex_image_filter='self'):
        params = {'Action': 'DescribeImages'}
    ...
    def get_image(self, ami):
        # get a single image object as returned by filtering by AMI
        params = {'Action': 'DescribeImages', 'ImageId.1' : ami}
        images = self._to_images(
            self.connection.request(self.path, params=params).object
        )
        image = images[0]
