...
class DebuggerTheme(CLI.ANSITheme):
    pass
...
class DebuggerCommands(CLI.BaseCommands):
    ...
    the .py suffix may be omitted."""
        ...
        line = checkline(filename, lineno)
        if line:
    ...
    then show only those local names."""
        ...
        if len(argv) > 1:
            for name in argv[1:]:
                try:
                    self._print("%25.25s = %s" % (name, _saferepr(f.f_locals[name])))
                except KeyError:
    ...
    Open your editor at the current location."""
        ...
        self._dbg.set_quit() # file possibly changed
